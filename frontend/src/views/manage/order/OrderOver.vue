<template>
  <a-modal v-model="show" title="订单详情" @cancel="onClose" :width="1000">
    <template slot="footer">
      <a-button key="over" @click="orderRefund(3)" :loading="loading">
        退款
      </a-button>
      <a-button key="back" @click="orderRefund(4)" type="danger">
        驳回
      </a-button>
    </template>
    <div style="font-size: 13px;font-family: SimHei" v-if="orderData !== null">
      ...
      <a-row style="padding-left: 24px;padding-right: 24px;">
        ...
        <a-col :span="8"><b>押金：</b>
          {{ orderData.rentPrice ? (orderData.rentPrice + '元') : '- -' }}
        </a-col>
      </a-row>
      <br/>
      <br/>
...
export default {
  ...
  methods: {
    orderRefund (status) {
      ...
      this.orderData.content = this.remark
      this.$put(`/cos/park-order-info/orderRefund`, this.orderData).then((r) => {
        this.$emit('success')
      })
    },
    overOrder (row) {
