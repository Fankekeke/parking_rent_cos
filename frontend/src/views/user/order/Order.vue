...
<!--        <a-button @click="batchDelete">删除</a-button>-->
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          ...
          <a-icon type="sliders" theme="twoTone" @click="orderAuditOpen(record)" title="押金申请" style="margin-left: 15px" v-if="record.status == 1"></a-icon>
...
export default {
  ...
  methods: {
    orderAuditOpen (row) {
      row.status = '2'
      this.$put(`/cos/park-order-info`, row).then((r) => {
        this.$message.success('订单申请押金退款成功')
        this.search()
      })
    },
    download (row) {
